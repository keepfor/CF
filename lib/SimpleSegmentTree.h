...
int main() {
  ...
  while (tt--) {
    ...
    SimpleSegmentTree<Info> st(n + 1);
    st.Set(0, {0});
    vector<int> dp(n + 1);
    for (int i = 1; i <= n; i++) {
      auto from = pref[i] - r;
      auto to = pref[i] - l;
      int a = int(lower_bound(pref.begin(), pref.end(), from) - pref.begin());
      int b = int(upper_bound(pref.begin(), pref.end(), to) - pref.begin());
      dp[i] = max(dp[i - 1], st.Query(a, b).mx + 1);
      st.Set(i, {dp[i]});
    }
    cout << dp[n] << '\n';
  }
