...
struct LCA {
  ...
  void build(int root = -1, bool build_rmq = true) {
    ...
    for (int i = 0; i < n; i++)
      if (parent[i] < 0) {
        tour_dfs(i, false);
        // Add a -1 in between connected components to help us detect when nodes
        // aren't connected.
        euler.push_back(-1);
      }
  ...
  pair<int, array<int, 2>> get_diameter() const {
    ...
    // We find the maximum of depth[u] - 2 * depth[x] + depth[v] where u, x, v
    // occur in order in the Euler tour.
  ...
  array<int, 2> get_center() const {
    ...
    return {get_kth_node_on_path(a, b, length / 2),
            get_kth_node_on_path(a, b, (length + 1) / 2)};
  }
  ...
  // Note: this is the LCA of any two nodes out of three when the third node is
  // the root. It is also the node with the minimum sum of distances to all
  // three nodes (the centroid of the three nodes).
  ...
  // Given a subset of k tree nodes, computes the minimal subtree that contains
  // all the nodes (at most 2k - 1 nodes). Returns a list of {node, parent} for
  // every node in the subtree sorted by tour index. Runs in O(k log k). Note
  // that all parents also appear as a node in the return value, and
  // nodes[0].first is the compressed root.
...
void run_case() {
    ...
    lca.build();
    vector<vector<array<int, 2>>> pairs_by_dist(N);

    for (int a = 0; a < N; a++)
        for (int b = a + 1; b < N; b++)
            pairs_by_dist[lca.get_dist(a, b)].push_back({a, b});

    vector<vector<int>> dp(N, vector<int>(N, 1));

    for (int d = 1; d < N; d++)
        for (auto &[a, b] : pairs_by_dist[d]) {
            if (d == 1) {
                if (S[a] == S[b])
                    dp[a][b] = dp[b][a] = 2;
            ...
            int a_neigh = lca.get_kth_node_on_path(a, b, 1);
            int b_neigh = lca.get_kth_node_on_path(b, a, 1);

            if (S[a] == S[b])
                dp[a][b] = dp[a_neigh][b_neigh] + 2;
            else
                dp[a][b] = max(dp[a_neigh][b], dp[a][b_neigh]);
