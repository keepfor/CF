...
// Note that T must be able to handle sums of values, not just individual
// values.
using T = int64_t;
...
struct splay_change {
  ...
  // Return the combined result of applying this splay_change followed by
  // `other`.
...
struct splay_node {
  ...
  friend int get_size(splay_node *x) { return x == nullptr ? 0 : x->size; }

  friend T get_max(splay_node *x) {
    return x == nullptr ? numeric_limits<T>::lowest() : x->maximum;
  }

  friend T get_sum(splay_node *x) { return x == nullptr ? 0 : x->sum; }
  ...
  void join() {
    ...
    // TODO: Add other operations here. Remember to store my_value in addition
    // to, e.g., min_value or max_value.
...
struct splay_tree {
  ...
  splay_tree(const vector<T> &values = {}) { init(values); }

  splay_tree(splay_node *node) { set_root(node); }

  splay_node *construct(const vector<T> &values, int start, int end) {
    if (start >= end) return nullptr;

    if (end - start == 1) return new_node(values[start]);
  ...
  // Note that splay(x) handles both pushing x and joining all nodes from x to
  // the root, inclusive.
  ...
  // Returns a splay_node pointer representing the first `count` nodes. If none,
  // returns `nullptr`.
  ...
  // Returns a splay_node pointer representing the last `count` nodes. If none,
  // returns `nullptr`.
  ...
  // Returns a splay_node pointer representing the index range [start, end). If
  ...
  splay_node *query_range(int start, int end) {
    ...
    if (after->parent != before) rotate_up(after);
  ...
  // Finds the end of the last subarray starting at `first` satisfying
  // `should_join` via binary search. should_join(splay_node *node, bool
  // single_node) -> bool Determines whether we should join with a node (if
  // single_node then just the node, else the subtree). If true, actually
  // performs the join.
  template <typename T_bool>
  int find_last_subarray(T_bool &&should_join, int first = 0) {
    if (!should_join(nullptr, false)) return first - 1;

    splay_node *current =
        first == 0 ? root : query_suffix_count(size() - first);
    ...
    while (current != nullptr) {
      ...
      if (!should_join(current->child[0], false)) {
        ...
        if (!should_join(current, true)) break;
    ...
    if (previous != nullptr) splay(previous);
...
void run_case(int test_case) {
  ...
  for (auto &e : E) {
    ...
    while (low < high) {
      ...
      int below = tree.find_last_subarray(
          [&](splay_node *node, bool single_node) -> bool {
  ...
  for (splay_node *node = tree.begin(); node != nullptr;
       node = tree.successor(node), index++) {
