...
// TODO: set this to false if it's unnecessary and the time limit might be
// tight. CHECK_OVERFLOW64 = true can run up to 2 times slower (particularly on
// CF).
...
// Returns twice the signed area formed by three points in a triangle. Positive
// when a -> b -> c is a left turn.
...
void run_case(int test_case) {
  ...
  for (auto &ant : ants) cin >> ant.x >> ant.y;
  ...
  for (int iter = 0; iter < ITERS; iter++) {
    ...
    for (int i = 0; i < N; i++)
      if (i != a && i != b && collinear(ants[a], ants[b], ants[i])) line++;
