    ...
    for (int i = 0; i < N; i++) {
      if (s >> i & 1) {
        for (int j = 0; j < N; j++) {
          if ((~s >> j & 1) && dis[i][j] != -1) {
            dp[s | 1 << j][j] =
                std::min(dp[s | 1 << j][j], dp[s][i] + dis[i][j]);
          }
        }
      }
    }
  ...
  for (int i = 0; i < N; i++) {
    ...
    if (S[i] == '?') {
      ...
      for (int j = 0; j <= std::min(cntq, cntv); j++) {
        res += comb.binom(cntq, j) * comb.binom(cntv, j) * comb.fac(j) *
               (26 - cntv + j) * pw[cntq - j];
      }
      ans -= res * dp[i + 1][2];
      cntq++;
      ...
      for (int j = 0; j <= std::min(cntq - 1, cntv - 1); j++) {
        res += comb.binom(cntq, j + 1) * comb.binom(cntv - 1, j) *
               comb.fac(j + 1) * pw[cntq - j - 1];
      }
      ans -= res * dp[i + 1][2];
      vis[S[i] - 'A'] = true;
      cntv--;
