...
int solve(vector<string> S, pint s, pint t, char ok, char ng) {
    const int N = S.size();
    auto f = [&](int i, int j) { return i * N + j; };

    shortest_path<int> sp(N * N);
    REP(i, N) REP(j, N) {
        for (const auto &d : grid_dxs) {
            int ni = i + d.first, nj = j + d.second;
            if (ni < 0 or ni >= N or nj < 0 or nj >= N) continue;
            int cost = (S.at(ni).at(nj) == ng);
            sp.add_edge(f(i, j), f(ni, nj), cost);
        }
    }
    sp.solve(f(s.first, s.second), f(t.first, t.second));
    return sp.dist[f(t.first, t.second)];
}
...
int main() {
    ...
    int ret1 = solve(S, pint(0, 0), pint(N - 1, N - 1), 'R', 'B');
    int ret2 = solve(S, pint(0, N - 1), pint(N - 1, 0), 'B', 'R');
