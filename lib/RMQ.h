...
// Warning: LCA uses nearly 170 MB for N = 500,000. If low on memory use
...
// For every i, finds the smallest j > i such that `compare(values[j],
// values[i])` is true, or `n` if no such j exists.
...
void run_case() {
  ...
  auto change = [&](int x, int delta) -> void {
    ...
    if (it != freq_set.end()) freq_set.erase(it);
  ...
  auto query = [&](int L, int R) -> int64_t {
    ...
    return suffix_sum[L] - suffix_sum[min_index] +
           int64_t(L + len - min_index) * ans[min_index];
  };
