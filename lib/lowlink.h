...
int main() {
  ...
  // Process the queries where original d(s[j], t[j]) does not coincides with
  // w[a[j]]
  dsu uf(n);
  int l = 0;
  for (int i = 0; i < m; i++) {
  ...
  for (int i = 0; i < m; i++) {
    vector<int>
        ls;  // The set of representative elements of the currenct connected
             // components that are connected to a weight-i edge
    for (int j : weight[i]) {
    ...
    while (it < q and w[a[ord[it]]] == i) {
      ...
      int ns =
          lower_bound(ls.begin(), ls.end(), uf.leader(s[now])) - ls.begin();
      int nt =
          lower_bound(ls.begin(), ls.end(), uf.leader(t[now])) - ls.begin();
      assert(ns != nt);
